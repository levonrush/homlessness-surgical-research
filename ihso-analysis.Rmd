...
wilcox.test(age ~ status, data = homeless_data)
...
con_tab %>% lapply(., fisher.test, alternative = 'greater')
```
...
homeless_data <- homeless_data %>%
  mutate(ln_los = log(los))

los_lm <- lm(ln_los ~ status + age + sex + da_disorder + mental_illness,
             data = homeless_data)
...
```{r, message=FALSE, warning=FALSE}
los_lm_final <- lm(ln_los ~ status + age + da_disorder + mental_illness,
                   data = homeless_data)
...
## get all the models together, derive their risk factors and plot them together

```{r}
# derive the risk factors from the model coefficients
risks_from_coefs <- function(model, risk_factors = F){
  
  # get the coefficients into a df
  model_coefs <- model %>%
    with(cbind(coefficients(.), confint(.))) %>% as.data.frame() %>%
    mutate_if(is.numeric, list(exp)) %>%
    mutate(variable = rownames(.)) %>%
    rename("effect_size" = "V1") %>%
    filter(variable != "(Intercept)")
  
  # # do you want to return the coefficients as risk factors?
  # if(risk_factors == T){
  #   
...
lapply(list(comp_glm_final, mort_glm_final, los_lm_final, icu_glm_final, daor_glm_final), risks_from_coefs, risk_factors = T)
```


```{r, message=FALSE, warning=FALSE}
graph_func <- function(model){
  
  # get the coefficients from the model
  model_coefs <- risks_from_coefs(model)

  # plot them out
  coef_plot <- ggplot(model_coefs, aes(x = variable, y = effect_size)) +
    geom_point() +
    geom_errorbar(aes(ymin = `2.5 %`, ymax = `97.5 %`), width = 0.1) +
    xlab(str_to_title(str_replace_all(model$terms[[2]], "[[:punct:]]", " "))) +
    ylab('') +
    ylim(0.5,7) +
    scale_x_discrete(labels = c("statushomeless" = "Homeless", "age" = "Age (10 years)", "sexM" = "Male", "da_disorderTRUE" = "Substance use disorder", "mental_illnessTRUE" = "Mental illness")) +
    coord_flip() +
    theme_minimal()
...
coefficient_graphs <- lapply(list(comp_glm_final, mort_glm_final, los_lm_final, icu_glm_final, daor_glm_final), graph_func)

coef_plots <- ggarrange(
  coefficient_graphs[[1]], coefficient_graphs[[2]], coefficient_graphs[[3]], coefficient_graphs[[4]], coefficient_graphs[[5]],
  ncol = 1,
  widths = rep(1, length(coefficient_graphs)),
  common.legend = T,
  legend = "bottom"
  )

coef_plots
annotate_figure(coef_plots,
                top = text_grob("Derived risk factors from models"),
                bottom = text_grob("Estimate"))
```
