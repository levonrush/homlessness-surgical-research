...
The following observational analysis has been designed to determine the impact of homelessness on surgical outcomes. To determine this impact, if any, we fit a number of linear models containing their housing status as well as other potentially confounding variables to determine whether there is an impact and quantify it.

The datasets relating to these outcomes were provided by the St Vincent's Hospital Sydney Coding Office (SVHSCO).
...
* Patient Mortality as `mortality`: a logical variable indicating whether the patient had a died during their hospital admission.
...
* Discharge at Own Risk as `daor`: a logical variable indicating whether the patient discharged themselves at their own risk.

## Complicaiton factors

The variable of interest was:

* Housing Status as`status`: the housing status of the patient undergoing surgery.

Other potentially confounding variables considered as part of the analysis were:

* Patient Age as `age`: a measurement of the patients age in years at time of admission.
...
The data is contained as a google sheet. The following routine fetches cleans and generates features from these datasets to create a analysis dataframe called `homeless_data`.
...
* `da_disorder`: a logical variable as defined by the inclusion of `assoc_diseases` beginning with `F1`.
* `mental_illness`: a logical variable as defined by the inclusion of `assoc_diseases` beginning with `F2`, `F3` or `F4`.
...
homeless_data <- homeless_data %>%
  ...
  mutate(complication = ifelse(str_detect(assoc_diseases, "T8."), T, F),
         ...
         mental_illness = ifelse(str_detect(assoc_diseases, "[F2|F3|F4]"), T, F),
         daor = ifelse(discharge_status == "Discharge at Own Risk", T, F)) 

# only keep relevant columns
homeless_data <- homeless_data %>%
  select(-assoc_diseases)
...
We observe that the variables `status` (P = `r comp_glm_summary$coefficients["statushomeless", 4]`) and `mental_illness` (P = `r comp_glm_summary$coefficients["mental_illnessTRUE", 4]`) don't contribute to the model. As such we eliminate these terms from the modelling to reach our final model.

```{r, message=FALSE, warning=FALSE}
comp_glm_final <- glm(complication ~ age + sex + da_disorder, 
                family = binomial(),
                data = homeless_data)
...
percent(exp(comp_glm_final$coefficients[["age"]])-1, accuracy = 0.01)
exp(confint(comp_glm_final)["da_disorderTRUE",])-1
```
...
To determine the effects on a patient's length of stay we fit linear models to our dataset.

To do this firstly we need to make a log transform to our outcome variable in order to fit the model properly.

```{r, message=FALSE, warning=FALSE}
# new log transformed outtcome variable
homeless_data <- homeless_data %>%
...
To determine the effects on a patient's length of stay in ICU we fit linear models to our dataset.
...
We observe that the variables `status` (P = `r icu_lm_summary$coefficients["statushomeless", 4]`), `sex` (P = `r icu_lm_summary$coefficients["sexM", 4]`) and `mental_illness` (P = `r icu_lm_summary$coefficients["mental_illnessTRUE", 4]`) don't contribute to the model. As such we eliminate these terms from the modelling to reach our final model.

```{r, message=FALSE, warning=FALSE}
icu_lm_final <- lm(icu_hrs_trans ~ age + da_disorder, 
             data = homeless_data)
...
* For each year increase of patients age, the length of stay in ICU is expected to increase by `r percent(exp(icu_lm_final$coefficients[["age"]])-1, accuracy = 0.01)` (95% CI = `r exp(confint(icu_lm_final)["age",])-1`, P = `r icu_lm_final_summary$coefficients["age", 4]`).
...
To determine the effects on patients risk to discharge themselves at their own risk post surgery we fit a logistic regression model to our dataset as so:
...
Given that all the variables in this model show significant effects we can determine that that:
...
* Patients with associated mental illnesses are `r percent(exp(daor_glm$coefficients[["mental_illnessTRUE"]])-1, accuracy = 0.01)` more likely to discharge themselves at their own risk than those without (95% CI = `r exp(confint(daor_glm)["mental_illnessTRUE",])-1`, P = `r daor_glm_summary$coefficients["mental_illnessTRUE", 4]`).

# Conclusions

By fitting sets of models to our outcomes variables we have been able to determine that the only effects homelessness has on surgical outcomes and increase to a patients length of stay and risk of discharge at own risk. All other analysed outcomes are associated with other confounding variables that are often associated with homelessness itself.
